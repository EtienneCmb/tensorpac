...
def ComputePac(pha, amp, idp, axis, nbins, p):
    ...
    # Mean Vector Length (Canolty, 2006)
    if idp == 1:
        return MVL(pha, amp, axis)

    # Kullback-Leiber divergence (Tort, 2010)
    elif idp == 2:
        return klDivergence(pha, amp, axis, nbins)

    # Heights ratio
    elif idp == 3:
        return HeightsRatio(pha, amp, axis, nbins)

    # ndPac (Ozkurt, 2012)
    elif idp == 4:
        return ndPac(pha, amp, axis, p)
...
def klDivergence(pha, amp, axis, nbins):
    """Kullback Leibler Divergence (Tort, 2010).
    ...
    abin = _kl_hr(pha, amp, axis, nbins)
...
def HeightsRatio(pha, amp, axis, nbins):
    """Pac Heights Ratio.
...
def ndPac(pha, amp, axis, p):
    ...
    npts = amp.shape[axis]
    # Normalize amplitude :
    amp = np.divide(amp - np.mean(amp, axis=axis, keepdims=True),
                    np.std(amp, axis=axis, keepdims=True))
    # Compute pac :
