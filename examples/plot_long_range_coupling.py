...
# and 100hz. By default, those datasets are organized as (n_trials, n_pts)
# where n_pts is the number of time points.
n = 20  # number of datasets
d1, time = pac_signals_tort(f_pha=10, f_amp=100, noise=1, n_trials=n,
                            n_pts=3000)
d2, time = pac_signals_tort(f_pha=10, f_amp=100, noise=3, n_trials=n, dpha=20,
                            damp=5, chi=.3, n_pts=3000)
...
# of the second
xpac12 = p.filterfit(1024, d1, d2).squeeze()
# Invert by taking the phase of the second dataset and the amplitude of the
#  first one :
xpac21 = p.filterfit(1024, d2, d1).squeeze()

# Plot signals and PAC :
plt.figure(figsize=(18, 12))
plt.subplot(2, 2, 1)
plt.plot(time, d1.squeeze().mean(0), color='k')
plt.xlabel('Time')
...
plt.subplot(2, 2, 2)
plt.plot(time, d2.squeeze().mean(0), color='k')
plt.xlabel('Time')
plt.ylabel('Amplitude [uV]')
